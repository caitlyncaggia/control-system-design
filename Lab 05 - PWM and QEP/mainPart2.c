...
float32 position;
float32 avgvoltage[2000];
float32 measuredposition [2000];
float32 adjvolt = 0;
float32 voltage = 0;
float32 zero = 0;
float32 high = 20;
...
interrupt void TimerISR(void)
{
    ...
    position = positioncounter * 2.0 * 3.14 / 1000.0;
    //float32

    //store data
    if(datacounter < 2000)
    {
        measuredposition[datacounter] = position;
        //float32                       float32
        avgvoltage[datacounter] = voltage;
        datacounter++;
    }
