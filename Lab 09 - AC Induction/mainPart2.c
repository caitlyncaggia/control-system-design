...
float32 uprev = 0.0; //average voltage
float32 yprev = 0.0; //measured position in rads
float32 sigmaprev = 0.0;
float32 xhat1prev = 0.0;
float32 xhat2prev = 0.0;
float32 u = 0.0; //average voltage
float32 y = 0.0; //measured position in rads
float32 ustar = 0.0;
float32 sigma = 0.0;
float32 xhat1 = 0.0;
float32 xhat2 = 0.0;
float32 unext = 0.0; //average voltage
float32 ynext = 0.0; //measured position in rads
float32 sigmanext = 0.0;
float32 xhat1next = 0.0;
float32 xhat2next = 0.0;
float32 thetae = 0.0;
float32 rho = 0.0;
float32 rhonext = 0.0;
...
interrupt void TimerISR(void)
{
    ...
    if (counter < 5000) //enter timer ISR every 1e-3sec, want requests every 0.5 sec
    {
        r = 0;
    }
    ...
    //determine alphav
    alphav = sqrt(vd*vd + vq*vq);

    //determine phiv
    phiv = atan2(vq, vd);

    //calculate voltages
    rhonext = fmod(rho + T*(gamma*gamma*Rr/R) * u/K, 6.28);
    thetae = fmod((Rs/R)*y + rho, 6.28);
    vA = 0.5*24 + sqrt(2.0/9.0)*alphav*cos(N*thetae + phiv - 3.14/6.0);
    vB = 0.5*24 + sqrt(2.0/9.0)*alphav*cos(N*thetae + phiv - 3.14/6.0 - 2.0*3.14/3.0);
    vC = 0.5*24 + sqrt(2.0/9.0)*alphav*cos(N*thetae + phiv - 3.14/6.0 + 2.0*3.14/3.0);
